...
class QueryService:
    ...
    def generate_search_queries(
        ...
        try:
            ...
            logger.info(
                f"Generated {len(result.get('search_queries', []))} queries for topic '{topic}'"
            )
            return result
...
Generate diverse, comprehensive queries that will capture the full breadth of research in the given topic."""
    ...
    def _build_prompt(
        ...
        if date_from or date_to:
            ...
            date_constraint += (
                "- Combine date filters with topic queries using AND operator\n"
            )
            date_constraint += '- Example: ti:machine AND ti:learning AND submittedDate:[20240101 TO 20241231]\n'
...
- Use proper arXiv API syntax (NO quotes, individual words with AND)
- Break phrases into individual words: "machine learning" → ti:machine AND ti:learning
- DO NOT include year constraints in queries
...
Focus on creating queries that will discover the most relevant and comprehensive set of papers for someone researching "{topic}"."""
    ...
    def _get_fallback_queries(self, topic: str) -> Dict:
        ...
        # Break topic into individual words for proper query construction
        topic_words = topic.split()
        if len(topic_words) > 1:
            title_query = "ti:" + " AND ti:".join(topic_words)
            abs_query = "abs:" + " AND abs:".join(topic_words)
            all_query = "all:" + " AND all:".join(topic_words)
        else:
            title_query = f"ti:{topic}"
            abs_query = f"abs:{topic}"
            ...
            "search_queries": [
                {
                    "query": title_query,
                    "priority": "high",
                    "description": f"Papers with '{topic}' words in title",
                },
                {
                    "query": abs_query,
                    "priority": "medium",
                    "description": f"Papers with '{topic}' words in abstract",
                },
                {
                    "query": all_query,
                    "priority": "low",
                    "description": f"Papers mentioning '{topic}' words anywhere",
                },
            ],
