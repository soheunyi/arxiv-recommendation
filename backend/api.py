...
try:
    ...
    from arxiv_recommendation.services.backup_service import BackupService
...
def create_response(data: Any, success: bool = True, message: str = None) -> Dict[str, Any]:
...
async def startup_event():
    ...
    try:
        ...
        try:
            from arxiv_recommendation.scheduler import start_scheduler
...
async def shutdown_event():
    ...
    try:
        ...
        # Stop scheduler
        try:
            from arxiv_recommendation.scheduler import stop_scheduler
...
async def get_papers(
    ...
    try:
        ...
        paginated_papers = papers[offset:offset + limit] if len(papers) > offset else []
    ...
    try:
        ...
        if rating_filter and rating_filter != "All":
            if rating_filter == "unrated":
                papers = [p for p in papers if not p.get("rating") or p.get("rating") == 0]
            else:
...
async def get_paper_stats():
    ...
    try:
        ...
        async with aiosqlite.connect(db_manager.db_path) as db:
            ...
            counts = {}
            tables = ["papers", "user_ratings", "paper_embeddings", "search_history", "recommendations_history"]
...
async def get_analytics_overview():
    ...
    try:
        ...
        for category in category_stats:
            ...
            avg_rating = stats["rating_sum"] / stats["rated"] if stats["rated"] > 0 else 0
            percentage = (stats["total"] / total_papers * 100) if total_papers > 0 else 0
        ...
        analytics = {
            ...
            "summary": {
                "most_common_rating": max(rating_distribution.items(), key=lambda x: x[1])[0] if any(rating_distribution.values()) else None,
                "total_categories": len(category_ratings),
                "best_rated_category": max(category_ratings.items(), key=lambda x: x[1]["average_rating"])[0] if category_ratings else None
            }
        }
...
async def search_papers(search_request: SearchRequest):
    ...
    try:
        ...
        if search_request.sortBy == "date":
            filtered_papers.sort(key=lambda x: x.get("published_date", ""), reverse=True)
...
async def generate_recommendations():
    ...
    try:
        ...
        # For now, return recent papers as recommendations
        # TODO: Implement actual recommendation engine with user preferences
...
async def preview_collection(collection_request: CollectionRequest):
    ...
    try:
        ...
        await collector.generate_queries(max_queries=collection_request.max_papers or 15)
...
async def generate_queries(request: dict):
    ...
    try:
        ...
        queries = query_service.generate_search_queries(topic, max_queries)
...
async def save_collection_config(request: dict):
    ...
    try:
        ...
        if not config_data or not filepath:
            raise HTTPException(status_code=400, detail="Config and filepath are required")
...
async def get_scheduler_status():
    ...
    try:
        from arxiv_recommendation.scheduler import get_scheduler
...
async def trigger_scheduled_task(task_name: str):
    ...
    try:
        ...
        valid_tasks = ["daily_collection", "daily_scoring", "preference_update", "cache_maintenance"]
        if task_name not in valid_tasks:
            raise HTTPException(status_code=400, detail=f"Invalid task name. Valid tasks: {valid_tasks}")
...
async def get_preference_mode():
    ...
    try:
        from arxiv_recommendation.preferences import PreferenceManager
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get preference mode: {str(e)}")

@app.put("/api/preferences/mode")
async def set_preference_mode(request: PreferenceModeRequest):
    """Set the user preference mode."""
    try:
        from arxiv_recommendation.preferences import PreferenceManager, PreferenceMode
        ...
        raise HTTPException(status_code=500, detail=f"Failed to set preference mode: {str(e)}")
...
async def get_preference_stats():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get preference stats: {str(e)}")
...
async def get_scoring_stats():
    ...
    try:
        from arxiv_recommendation.scoring_service import ScoringService
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get scoring stats: {str(e)}")
    ...
    try:
        ...
        if request.clean_db:
            ...
            cleanup_result = await collector.clean_database(
                ...
                granular_options=cleanup_options
            )
            ...
            if not cleanup_result["success"]:
                ...
                raise HTTPException(status_code=500, detail=f"Database cleanup failed: {cleanup_result.get('error')}")
            
            logger.info(f"Database cleanup completed: {cleanup_result['total_records_removed']} records removed")
        ...
        for i, query_info in enumerate(queries):
            try:
                ...
                # Execute query
                query_papers = await collector.search_by_query(query_info, request.max_papers)
